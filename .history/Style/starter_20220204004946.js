...
btn.addEventListener('click',function(e){
  ...
  if(text.length == 0 || text.includes("@")==false){
    ...
    formInp.classList.remove('true');

  }
  else{
    formInp.classList.remove('false');
    formInp.classList.add('true');
...
const arrcodianList=document.querySelectorAll('.arcodian');
[...arrcodianList].forEach(item=>item.addEventListener('click',function(e){
  const icon=e.target.querySelector('button i');
  icon.classList.toggle('fa-angle-down');
  icon.classList.toggle('fa-angle-up');
  const arcodianContent=e.target.nextElementSibling;
 
  // arcodianContent.classList.toggle('active');
  if(arcodianContent.offsetHeight==0){
    arcodianContent.style.height=`${220}px`;

  }
  else{
  arcodianContent.style.height=`0`;
